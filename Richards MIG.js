(function() {
    ...
    mig.innerHTML = `
        ...
        <div id="migTitle">Richard.MIG</div>
    ...
    style.textContent = `
        #liamMIG {
            ...
            background: rgba(120, 0, 0, 0.55); /* REALLY dark red */
        ...
        #migTitle {
            background: #D40000; /* Coca-Cola red */
            color: white;
            padding: 8px;
            cursor: move;
            font-weight: bold;
    ...
    function attachSwitchLogic() {
        document.querySelectorAll("#liamMIG .switchBox").forEach(box => {
            box.addEventListener("click", () => {
                ...
                console.log("Richard.MIG:", key + " =", isOn);
            });
        });
    }
    attachSwitchLogic();
    ...
    function drawNodes() {
        ...
        nodes.forEach(n => {
            ...
            ctx.fillStyle = "rgba(255,255,255,0.9)"; // white nodes
            ctx.beginPath();
            ...
            if (distMouse < MAX_DIST) {
                ctx.strokeStyle = "rgba(212,0,0," + (1 - distMouse / MAX_DIST) + ")";
                ctx.beginPath();
                ctx.moveTo(n.x, n.y);
            ...
            nodes.forEach(n2 => {
                ...
                if (dist < MAX_DIST) {
                    ctx.strokeStyle = "rgba(212,0,0," + (1 - dist / MAX_DIST) + ")";
                    ctx.beginPath();
                    ctx.moveTo(n.x, n.y);
